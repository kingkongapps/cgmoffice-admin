window.addEventListener('CUC001001_M01', () => {
	CUC001001_M01.init();
})

const CUC001001_M01 = {
	targetElId: 'CUC001001_M01',
	init: function() {
		console.log(`>>> ${this.targetElId}`);

		// 약관코드수신내역조회 상세팝업
		CMMN.include("CUC001001_P01");
	...
	// 팝업열기
	CUC001001_P01_open: async function() {
		CUC001001_P01.open(
	...
	search_01: async function() {
		...
		const response = await CMMN.api.post('/api/cuc/clusCd/clusResHisList', { params });
		...
		document.querySelector("#CUC001001_M01_totalCount").innerText = '총 건수 : ' + CMMN.priceToString(this.data.paginator_01.totalCount, ',');
	},
	// 페이지 이동
