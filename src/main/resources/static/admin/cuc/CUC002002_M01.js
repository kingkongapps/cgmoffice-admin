window.addEventListener('CUC002002_M01', () => {
	CUC002002_M01.init();
})

const CUC002002_M01 = {
	targetElId: 'CUC002002_M01',
	init: function() {
		console.log(`>>> ${this.targetElId}`);

		// 개별약관합본테스트정보이력조회 상세팝업 [CLOB 필드 조회 시간 때문에 미리 include]
		CMMN.include("CUC002002_P01");
	...
	// 팝업열기
	CUC002002_P01_open: async function() {
		CUC002002_P01.open(
	...
	search_01: async function() {
		...
		const params = {
			...
			nprdtNm: document.querySelector(`#${this.targetElId} [name='nprdtNm']`).value,
			rcmYmd: document.querySelector(`#${this.targetElId} [name='rcmYmd']`).value.split('.').join(''),
			pageConfig: this.data.pageConfig_01,
		}

		const response = await CMMN.api.post('/api/cuc/clusCd/clusMergeTxtHisList', { params });
		...
		document.querySelector("#CUC002002_M01_totalCount").innerText = '총 건수 : ' + CMMN.priceToString(this.data.paginator_01.totalCount, ',');
	},
	// 페이지 이동
