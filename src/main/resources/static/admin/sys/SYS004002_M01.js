...
window.addEventListener('SYS004002_M01', () => {
	SYS004002_M01.init();

	CMMN.include("SYS004002_P01"); // 팝업
})

const SYS004002_M01 = {
	targetElId: 'SYS004002_M01',
	init: function() {
	...
	search: async function() {
		...
		const searchMsgClcd = document.querySelector(`#${this.targetElId} [name='searchMsgClcd']`).value;
		const searchData = document.querySelector(`#${this.targetElId} [name='searchData']`).value;

		const params = {
			msgType : searchMsgClcd == '1' ? searchData : "",
			msgNm : searchMsgClcd == '2' ? searchData : "",

			pageConfig: this.data.pageConfig,
		}
		const response = await CMMN.api.post('/api/sys/SysMsgMng/getListPage', { params });
		...
		document.querySelector(`#${this.targetElId} [name='SYS004002_M01_totalCount']`).innerText = '총 건수 : ' + CMMN.priceToString(this.data.paginator.totalCount);
	},
	...
	delete: async function() {
		...
		if(!await CMMN.confirm('선택한 메시지를 삭제하시겠습니까?')) {
			return;
		}

		const params = this.data.gridSelectedDataList;

		await CMMN.api.post('/api/sys/SysMsgMng/deleteList', { params });
	...
	reset : async function(){
		...
		document.querySelector(`#${this.targetElId} [name='searchMsgClcd']`).value = '1'; //디폴트값으로 변경
