...
window.addEventListener('SYS005001_M01', () => {
	SYS005001_M01.init();
})

const SYS005001_M01 = {
	targetElId: 'SYS005001_M01',
	init: function() {
	...
	data: {
		...
		gridOptions_01: {
			columnDefs: [  // grid 컬럼 정의
				...
				{
					headerName: '상품중지일',
					valueGetter: 'data.pmStopYmd ? moment(data.pmStopYmd, "YYYYMMDD").format("YYYY.MM.DD") : ""',
					field: 'pmStopYmd',
					minWidth: 120,
				},
				{
	...
	search_01: async function() {
		...
		const response = await CMMN.api.post('/api/sys/condPrdtMng/getListPage', { params });
	...
	excelDownLoad_01: async function() {
		...
		const response = await CMMN.api.post('/api/sys/condPrdtMng/excelDownLoad_01', options);
	...
	excelUpLoad_01: async function() {
		...
		await CMMN.api.post('/api/sys/condPrdtMng/excelUpLoad_01', { params });
	...
	setPrdtList_02: async function() {
		...
		const response = await CMMN.api.post('/api/sys/condPrdtMng/getList', { params });
		console.log('>>> response:', response);
	...
	search_02: async function() {
		...
		if(prdtCd) {
			...
			document.querySelector(`#${this.targetElId} [name='${this.targetElId}_totalCount_2']`).innerText = '총 건수 : ' + CMMN.priceToString(data.length);
		} else {
			this.data.gridOptions_02.api.setRowData([]);
		}
	},
	// 약관구성등록 엑셀다운로드
	excelDownLoad_02: async function() {
		// 함수별 권한제어 (I:insert, S:select, U:update, D:delete, E:excel)
		if(CMMN.chkAuthBtn(this.targetElId, "S")) return;
		...
		const response = await CMMN.api.post('/api/sys/condPrdtMng/excelDownLoad_02', options);

		CMMN.downloadFileProc(response);
	},
	// 약관구성등록 엑셀업로드
	excelUpLoad_02: async function() {
		// 함수별 권한제어 (I:insert, S:select, U:update, D:delete, E:excel)
		if(CMMN.chkAuthBtn(this.targetElId, "E")) return;
		...
		const params = { excelFile };
		CMMN.api.post('/api/sys/condPrdtMng/excelUpLoad_02', { params }).then(data => {
			el_excelFile.value = '';  // 엑셀파일 첨부input 을 초기화 한다.
			this.pageToFirst_01();
		}).catch(err => {
