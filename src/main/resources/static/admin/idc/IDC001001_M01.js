window.addEventListener('IDC001001_M01', () => {
	IDC001001_M01.init();
})

const IDC001001_M01 = {
	targetElId: 'IDC001001_M01',
	init: function() {
		console.log(`>>> ${this.targetElId}`);

		// 개별약관생성조합내역조회 상세팝업
		CMMN.include("IDC001001_P01");
	...
	configUI: function() {
		// 계약일자
		$(`#${this.targetElId} [data-field='crtDtm']`).datepicker({
		...
		.on("changeDate", (e) => { // 날자의 값이 변경될때마다 data.params 의 해당 데이터를 실시간 변경하도록 설정
			...
			document.querySelector(`#${this.targetElId} [name='crtDtm']`).value = selectedDate.split('.').join('');
		});
	...
	// 팝업열기
	IDC001001_P01_open: async function() {
		IDC001001_P01.open(
	...
	search_01: async function() {
		...
		const params = {
			apcno: document.querySelector(`#${this.targetElId} [name='apcno']`).value,
			cmpnyNm: document.querySelector(`#${this.targetElId} [name='cmpnyNm']`).value,
			crtDtm: document.querySelector(`#${this.targetElId} [name='crtDtm']`).value.split('.').join(''),
			pageConfig: this.data.pageConfig_01,
		}

		const response = await CMMN.api.post('/api/idc/indClus/indUnnHisList', { params });
		...
		document.querySelector("#IDC001001_M01_totalCount").innerText = '총 건수 : ' + CMMN.priceToString(this.data.paginator_01.totalCount);
	},
	// 페이지 이동
