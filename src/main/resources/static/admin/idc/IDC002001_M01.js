window.addEventListener('IDC002001_M01', () => {
	IDC002001_M01.init();
})

const IDC002001_M01 = {
	targetElId: 'IDC002001_M01',
	init: function() {
			...
			columnDefs: [  // grid 컬럼 정의
				{
					headerName: '항목',
					minWidth: 150,
					valueGetter: 'CMMN.getCmmnCdNm("CLUS_ITM_CLCD", data.clusItmClcd) + " (" + data.clusItmClcd + ")"',
					field: 'clusItmClcd',
				},
				{
					headerName: '주계약특약명',
	...
	search_01: async function() {
		...
		if(prdtCd) {
			...
			const response = await CMMN.api.post('/api/idc/indClus/indUnnDiffInfo', { params });
			...
			document.querySelector(`#${this.targetElId} [name='IDC002001_M01_totalCount_1']`).innerText = '총 건수 : ' + CMMN.priceToString(this.data.paginator_01.totalCount);
		} else {
			this.data.gridOptions_01.api.setRowData([]);
		}
	},
	// 주계약 select box 셋팅
	setPrdtList_01: async function() {
